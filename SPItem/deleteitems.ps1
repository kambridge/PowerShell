#Delete Items
#Description: WARNING! Deletes items in list.
#Notes: Change file path ($websfile) and Get-Content ($config)

#modify script for blog

[string] $websfile = "D:\Scripts\patsurl.xml"
[xml] $config = Get-Content $websfile
...
StartDeletion -url "<url>" -listName "Workflow History" -caml "<Query></Query>" -action "NotUsed" -rowlimit 5000
